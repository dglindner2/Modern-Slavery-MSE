...
make_AIC_stepwise_table1 <-
  function ()
  {
    ...
    zn2 = rep(c("Main effects only", "AIC threshold 0.1%", "AIC threshold 5%"),
              each = 3)
    ...
    for (j in (1:3)) {
      ...
      z[3 * j - 1, ] = MSEfit(UKdat_5, zmain[j], zpthresh[j], returnfit =
                                F)
      z[3 * j, ] = MSEfit(UKdat_4, zmain[j], zpthresh[j], returnfit =
                            F)
    }
    z = round(z / 1000, 1)
  ...
  function (burnin0 = 100000)
  {
    ...
    dimnames(zout) = list(
      c(
        ...
        "UK four lists",
        "Netherlands",
        "Netherlands five lists",
        "New Orleans",
        "NewOrleans five lists",
        "Kosovo"
      ),
      c("2.5%", "10%", "50%", "90%", "97.5%")
    )
...
make_madyor_table <-
            ...
            ...)
  {
    ...
    if (plotpost)
      plotPosteriorN(postmax, ytot)
    zz = apply(postmax, 2, sum)
    if (returnmaxmodprob)  {
      model_posterior_probs = apply(postmax, 1, sum)
      cat(
        "Posterior probability of most probable model for dataset",
        deparse(substitute(zdat)),
        "and maximum size",
        maxpop,
        ":",
        round(max(model_posterior_probs), 2),
        "\n"
      )
    }
    py = cumsum(zz)
    quantiles = approx(x = py,
                       y = ytot,
                       xout = c(0.025, 0.1, 0.5, 0.9, 0.975))
...
make_madyor_table_script <-
  function ()
  {
    ...
    dimnames(zout) = list(
      c(
        "UK five lists max 40K",
        "UK four lists",
        "Netherlands five lists",
        "Netherlands five lists max 200K",
        "NewOrleans five lists",
        "Kosovo"
      ),
      c("2.5%", "10%", "50%", "90%", "97.5%")
...
#' @export
make_MCMC_table <-
  function (zdata = UKdat,
            sigthreshseq = c(0, 2, 5),
            ...)
  {
    require(MCMCpack)
    ...
    for (j in (1:(4 * nsig))) {
      ...
      zr = MCMCfit(
        zdata,
        priorprec = priorprec1[j],
        sigthresh = sigthresh1[j],
        totalpopest = T,
        ...
      )
      zout = rbind(zout, c(priorprec1[j], sigthresh1[j],  quantile(
        zr[, 1], probs = c(0.025, 0.1, 0.5, 0.9, 0.975)
      )))
    }
    zout = zout[, -1]
    zout[, -1] = round(zout[, -1] / 1000, 1)
    dimnames(zout)[[1]] = c("Main", rep(c(
      "Uniform", "Variance 10", "Variance 1", "Variance 0.1"
    ), nsig))
...
mergelists <-
  function(zmse,
    ...
    if (nmerge > 2) {
      for (j in (2:nmerge))
        zmse = mergelists(zmse, im[1], im[j] + 2 - j, rowclean = F)
    } else {
      zz = zmse
      zn = dimnames(zz)[[2]]
      ii = im[1]
