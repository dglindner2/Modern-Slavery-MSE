...
**Usage recommentation:** Please allow one minute for the first plot to appear, before begining interacting with the application.
...
This tab assesses the reliability of Silverman's MSE approach through bootstrap simulations. Silverman developped an R package called "moderslavmse" (2018) to reproduce the MSE method used in his original paper (stepwise assuming only to way interactions). Here, We first reproduce Silverman's model on either of the three UK datasets. From this model, we generate datasets on which we fit new models with the same method. We finally plot estimates extracted from each simulated model and display coverage based on  [Silverman et al. (2015)](https://rss.onlinelibrary.wiley.com/doi/full/10.1111/j.1740-9713.2015.00824.x) in a table. Note: By clicking the "use pre-processed data" box, the user has the possibility to generate faster results using pre-processed simulations.
We can observe low coverage, especially when dealing with six or five lists, which shows that Confidence interval produced might be too narrow. This low coverage and the instability of the model question the reliability of the MSE method used by Silverman to estimate the number of victims of modern slavery.
